...
- Investigate why `discountType` is not propagated from Rialto to MediaHouse in the initial placement read (`tc_getMHTC03`).
- Verify commission/pricing mapping after MH patches placement (`tc_getMHTC03a`).
- Check `statusFlags` and `commissionAmount` sync on the Rialto readback after placement change (`tc_getIntegrationRialto05b`).
- Note: these same 3 tests have been failing since build #246 — regression predates this run.
