...
Build: `automationrunCAI-RIALTOB2A-IntegrationTesting-Internal-trunk` #137  
Date: 2026-07-12 21:03 UTC  
Status: UNSTABLE  
Total Tests: 513  
Passed: 424  
Failed: 89  
...
- TC6 changes the date and the size — User perform CASS GET API (REGRESSION)
- TC7 Change Date, Size & Placement — User perform CASS GET API (REGRESSION)
...
**Failure Pattern:**
`Index 13 out of bounds for length 13` in GET step; prevents `odIds` from being stored in `TestContext`; all subsequent POST/patch steps fail with `No MH odIds found in TestContext`.
...
- TC22 in MH change from Full page to uppslag — User perform CASS POST API (REGRESSION: packageId, placementId, prodCode order)
...
## Path parameter errors and basket setup failures

**Affected Features:**
- `rialtoB2A(CASS)TestCase1.feature`, `rialtoB2A(CASS)TestCase23.feature`, `rialtoB2A(CASS)TestCase24.feature`, `rialtoB2A(CASS)TestCase29.feature`

**Affected Scenarios:**
- TC1 and TC2 — User perform CASS POST API (basket not found); User perform CASS GET API (×2, mhBasketOrderId undefined); User perform CASS POST API (×2, cascade)
...
**Failure Pattern:**
TC1: `Basket not found for campaign: TestCase01 expected [false] but found [true]` cascades to `mhBasketOrderId` path parameter errors. TC24/TC29: Rialto GET steps have swapped or missing path parameters (`agencyPrisaId`/`uuid`). TC23: HTTP 500 `Transaction rolled back because it has been marked as rollback-only`.

**Evidence:**
- TC1 POST: `Basket not found for campaign: TestCase01 expected [false] but found [true]`
- TC24 Rialto: `Redundant path parameters: agencyPrisaId=6294. Undefined path parameters: uuid`
- TC29 Rialto: `Redundant path parameters: uuid=310d1f57-... Undefined path parameters: agencyPrisaId`
...
- TC27 Magazine (change date) — RIALTO Verify created order (assertion format `[[v]]` vs `[v]`); MEDIAHOUSE Update order lines (issueDate); MEDIAHOUSE Verify updated basket; RIALTO Verify reverted state
...
**Failure Pattern:**
Post-change GET validations show commission, priceNet, and totalInclVat deviations without the `discountType=null` signature. TC27/TC28 Rialto steps fail on assertion format (`[[v]]` vs `[v]`). TC27 also shows an issueDate mismatch (`2026-08-19` vs `2026-08-26`). TC35 has MH basket ID vs Agency Prisa ID mismatch.
...
- TC28 updated: `commission` expected `155.00` found `250.00`; `totalInclVat` expected `6056.25` found `5937.50`
- TC35: `MH basket ID (orBoxid) [6304] does not match Agency Prisa ID [6305]`
- TC37 original: `totalInclVat` expected `7368.00` found `7614.00`
...
- 89 failing validations grouped above across MH GET checks, MH/Rialto POST/PATCH steps, and Rialto follow-up GET checks.
