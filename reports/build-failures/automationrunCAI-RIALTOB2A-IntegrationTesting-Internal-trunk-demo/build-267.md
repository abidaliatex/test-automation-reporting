...
- These failures have been recurring since build #246; confirm whether the expected values for `getMHB2A.csv` and `getRialtoB2A.csv` have been deliberately left stale or are awaiting a fix.
- Verify whether `discountType`, commission, and depth fields are intentionally different from expectations after the API v88 upgrade.
