...
Build: `automationrunCAI-RIALTOB2A-IntegrationTesting-Internal-trunk-demo` #268  
Date: 2026-07-05 21:10 UTC  
Status: UNSTABLE  
Total Tests: 14  
Passed: 11  
Failed: 3  
...
**Failure Pattern:**
Rialto GET validation did not reflect the expected placement-derived depth, status flags, and commission values after the update.
...
- These failures have persisted since build #246; confirm whether the test expectations require updating to match a deliberate service change, or whether a regression needs investigation.
- Re-check expected values in `getMHB2A.csv` and `getRialtoB2A.csv` for `tc_getMHTC03`, `tc_getMHTC03a`, and `tc_getIntegrationRialto05b`.
