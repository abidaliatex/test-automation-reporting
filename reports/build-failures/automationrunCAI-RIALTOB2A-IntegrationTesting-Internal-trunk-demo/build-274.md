...
Build: `automationrunCAI-RIALTOB2A-IntegrationTesting-Internal-trunk-demo` #274
Date: 2026-07-10 10:44 UTC
Status: UNSTABLE
Total Tests: 14
Passed: 11
Failed: 3
...
### MediaHouse discount type and pricing assertions diverge from expected values
...
**Failure Pattern:**
MediaHouse GET validations fail because `discountType`, VAT, commission, and pricing totals differ from expected CSV fixture values after placement creation and change.
...
### Rialto post-change placement depth, commission, and status flag assertions diverge from expected values
...
**Failure Pattern:**
Rialto GET validation after placement update returns wrong `commissionAmount`, `depth`, and empty `statusFlags`; price fields are returned without decimal formatting (`250000.0` vs `250000.00`).

**Evidence:**
- `orderHeader.statusFlags` expected `[PRELIMINARY]` found `[]` — order status not set to PRELIMINARY after change.
...
- Price fields use truncated decimal format (`250000.0`) instead of expected two-decimal format (`250000.00`).
