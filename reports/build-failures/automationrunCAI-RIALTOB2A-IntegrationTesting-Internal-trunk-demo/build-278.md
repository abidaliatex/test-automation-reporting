...
Build: `automationrunCAI-RIALTOB2A-IntegrationTesting-Internal-trunk-demo` #278  
Date: 2026-07-10 14:10 UTC  
Status: UNSTABLE  
Total Tests: 14  
Passed: 11  
Failed: 3  
...
### MediaHouse discountType inversion and pricing mismatch
...
**Failure Pattern:**
`discountType` is `null` pre-change (expected `RIALTO`) and `RIALTO` post-change (expected `NONE`); all downstream pricing (VAT, totals) diverges from fixture values.
...
### Rialto placement dimension, status, and commission mismatch
...
**Failure Pattern:**
Rialto GET after placement change returns wrong `depth`, empty `statusFlags`, and significantly lower `commissionAmount`; numeric formatting differences also cause assertion failures.

**Evidence:**
- `orderHeader.statusFlags` expected `[[PRELIMINARY]]` found `[[]]`.
- `orderAdDetails[0].depth` expected `[184]` found `[372]` — placement update not applied.
...
- Numeric formatting mismatch: `priceGross` expected `[250000.00]` found `[250000.0]`.
