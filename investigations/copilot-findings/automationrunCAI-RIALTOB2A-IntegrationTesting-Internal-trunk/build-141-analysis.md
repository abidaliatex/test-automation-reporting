# Root Cause Analysis — `automationrunCAI-RIALTOB2A-IntegrationTesting-Internal-trunk` Build #141

**Source Report:** [build-141.md](../../../reports/build-failures/automationrunCAI-RIALTOB2A-IntegrationTesting-Internal-trunk/build-141.md)

---

## Summary

Build #141 (2026-07-16 21:03 UTC) produced **88 failures** across 513 tests (pass rate 82.8%). Failure count is unchanged from build #139 (88). All five chronic failure groups from prior builds persist; no previously failing groups were resolved and no new root causes were introduced.
...
- MH discountType propagation service (persistent regression since build #134)
...
- Test fixtures — `getMHB2A.csv`, `getRialtoB2A.csv` (stale post-regression expected values for pricing fields)
...
1. **Fixture refresh:** Update `getMHB2A.csv` and `getRialtoB2A.csv` expected values to reflect current service responses for `discountType`, `discountAmount`, commission, VAT, and `statusFlags`. This would resolve the majority of failures (groups 1 and 2).
