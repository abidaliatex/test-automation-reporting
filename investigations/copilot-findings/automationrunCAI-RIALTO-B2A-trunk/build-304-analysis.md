...
**Source report:** [build-304.md](../../reports/build-failures/automationrunCAI-RIALTO-B2A-trunk/build-304.md)

---

## Summary

Build 304 of `automationrunCAI-RIALTO-B2A-trunk` is UNSTABLE with 10 failures out of 17 tests (41.2% pass rate). All failures are confined to the `rialtoB2A(CASS)` feature. The pattern is identical to builds 302 and 303 — every CASS API endpoint returns HTTP 404, and downstream steps that depend on a `uuid` from a prior response then fail with cascade errors.
...
- Monitor for recurring 404s across consecutive builds (builds 302, 303, and 304 show the same failure pattern) to detect persistent environment outages early.
