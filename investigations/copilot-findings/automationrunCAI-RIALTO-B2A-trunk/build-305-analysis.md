# Root Cause Analysis — Build 305

**Source report:** [build-305.md](../../reports/build-failures/automationrunCAI-RIALTO-B2A-trunk/build-305.md)

---

## Summary

Build 305 of `automationrunCAI-RIALTO-B2A-trunk` is UNSTABLE with 10 failures out of 17 tests (41.2% pass rate). All failures are confined to the `rialtoB2A(CASS)` feature and are identical in pattern to builds 302–304 — every CASS API endpoint returns HTTP 404, and downstream steps that depend on a `uuid` extracted from a prior response cascade into additional errors.
...
- Monitor for recurring 404s across consecutive builds (builds 302–305 show the same failure pattern) to detect persistent environment outages early.
