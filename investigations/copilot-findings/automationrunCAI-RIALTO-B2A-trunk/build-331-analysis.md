# Investigation — automationrunCAI-RIALTO-B2A-trunk #331

Source report: [build-331.md](../../../reports/build-failures/automationrunCAI-RIALTO-B2A-trunk/build-331.md)

---

## Build Summary

Build: automationrunCAI-RIALTO-B2A-trunk #331  
Total Tests: 17  
Passed: 7  
Failed: 10  
Pass Rate: 41.2%
...
- These tests depend on a `uuid` extracted from a prior POST response that returned 404 instead of a valid body
...
## Summary
- The same two failure groups persist from preceding runs (#329, #330): direct 404 responses on CASS routes, followed by dependent scenario failures when no `uuid` is produced.

## Root Cause
- The `/api/870/v1/cass/` routes were unavailable during this run. The `crossad-integration-ws` service may not be deployed or is mis-routed at API version `870`.

## Affected Components
- `crossad-integration-ws` CASS API routes (all sub-paths under `/api/870/v1/cass/`)
- `rialtoB2A(CASS)` POST and GET flows
