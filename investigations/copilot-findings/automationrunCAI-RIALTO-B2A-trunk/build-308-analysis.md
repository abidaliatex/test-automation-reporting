...
Source report: [build-308.md](../../../reports/build-failures/automationrunCAI-RIALTO-B2A-trunk/build-308.md)

## Summary

- Build `automationrunCAI-RIALTO-B2A-trunk` #308 finished `UNSTABLE` with 10 failed tests out of 17 (pass rate 41.2%).
...
- This failure signature is identical to builds 305, 306, and 307, indicating a persistent environment issue.

## Root Cause

- **Primary:** Multiple CASS API routes under `/agency/crossad-integration-ws/api/870/v1/cass/` returned `HTTP 404 – Not Found`, indicating the CASS service is unavailable or its routes are not mapped on the test environment.
...
- Track repeated identical failure signatures across consecutive builds and alert on recurrence.
